...
import com.sdapps.wellbeing.core.CustomProgressDialog
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val mainMenu = mutableListOf(
            MenuBO(
                ...
                30,
                isTime = true,
                ContextCompat.getDrawable(this, R.drawable.icon1)?.toBitmap()
            ),
            MenuBO(
                CHALLENGE,
                5,
                isTime = false,
                ContextCompat.getDrawable(this, R.drawable.task)?.toBitmap()
            ),
            MenuBO(
                SYNC,
                0,
                false,
                ContextCompat.getDrawable(this, R.drawable.sync_icon)?.toBitmap()
            )
        )
    ...
    override fun handleCardClick(moduleName: String, menuBO: MenuBO) {
        ...
        if (moduleName.equals(SYNC, ignoreCase = true)) {
            if (isNetworkConnected()) {
                ...
                Toast.makeText(this@MindfulnessActivity, "Connect to network", Toast.LENGTH_LONG)
                    .show()
            }
...
//        date : {type: String, required: true}
            val sharedPreferences: SharedPreferences =
                getSharedPreferences(activityTracker, MODE_PRIVATE)
            val jsonObject1 = JSONObject().apply {
                ...
                put(
                    CURRENT_DATE,
                    sharedPreferences.getString(CURRENT_DATE, "")!!.replace("'", "\"")
                )
            }
